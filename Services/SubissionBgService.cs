...
public class SubissionBgService : ISubissionBgService
{
    ...
    private readonly ITraineeDirectoryClient _traineeDirectoryClient;
    public SubissionBgService(AppDbContext context, IConfiguration configuration, ILocalFileStorage localFileStorage, ITraineeDirectoryClient traineeDirectoryClient)
    {
        _context = context;
        _configuration = configuration;
        _fileStorageService = localFileStorage;
        _traineeDirectoryClient = traineeDirectoryClient;
    }
    public async Task GetFileMetaData(long docId, string messageId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new MaxAttemptExeption(attemptCount, err.Message, true);
            }
        ...
        catch (MaxAttemptExeption e)
        {
            ...
            processingJob.Status = "Failed";
            await _context.SaveChangesAsync();
            throw new MaxAttemptExeption(e.AttemptCount + 1, e.Message, e.RetryReque);
        }
    }
}